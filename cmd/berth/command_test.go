...
func TestSearchSpecificAgent(t *testing.T) {
	...
	fake.SetResponse(`docker inspect --format {{index .Config.Labels "berth.repo-display"}}`, "org/repo\n")
...
func TestServerRequestHandlers(t *testing.T) {
	...
	home := t.TempDir()
	t.Setenv("HOME", home)
	actionsPath := filepath.Join(home, ".berth", "actions.toml")
...
`), 0o600); err != nil {
	...
	if !strings.Contains(runOut, "agent-codex-reviewer") ||
		!strings.Contains(runOut, "BERTH_PROMPT_B64") ||
		!strings.Contains(runOut, "berth.auth=shared") ||
		!strings.Contains(runOut, "berth.docker=off") {
...
func TestHandoffToWorktree(t *testing.T) {
	...
	worktreePath := "/Users/florian/.berth/worktrees/" + containerName
	fake.SetResponse("docker ps -a --filter name=^agent-", containerName+"\n")
	fake.SetResponse(`docker inspect --format {{index .Config.Labels "berth.worktree"}}`, worktreePath+"\n")
...
`
	...
	if cmds := fake.CommandsMatching("docker cp /tmp/berth-patch-"); len(cmds) == 0 {
		t.Fatal("expected docker cp for staged patch")
	}
	stageCmds := fake.CommandsMatching("git apply --cached --whitespace=nowarn /tmp/berth-patch-")
	if len(stageCmds) == 0 {
		t.Fatalf("expected staged patch apply command, got %#v", fake.Log)
	}
	if strings.Contains(strings.Join(stageCmds[0], " "), "BERTH_PATCH") {
	...
	if cmds := fake.CommandsMatching("git apply --reverse --whitespace=nowarn --check /tmp/berth-patch-"); len(cmds) == 0 {
		t.Fatalf("expected revert preflight with patch file, got %#v", fake.Log)
	}
	if cmds := fake.CommandsMatching("git apply --reverse --whitespace=nowarn /tmp/berth-patch-"); len(cmds) == 0 {
...
func TestRunLogs_StoppedContainerUsesCopy(t *testing.T) {
	...
	fake.SetResponse("bash -c find", "/tmp/berth-logs-test/.claude/sessions/test.jsonl\n")
...
func TestCleanupAuthRemovesSharedAndIsolatedAuthVolumes(t *testing.T) {
	...
	fake.SetResponse("docker volume ls --filter name=berth- --format {{.Name}}", "berth-claude-auth\nberth-codex-gh-auth\n")
	fake.SetResponse("docker volume ls --filter label=berth.type=auth --format {{.Name}}", "agent-claude-worker-auth\n")
	...
	for _, want := range []string{
		"docker volume rm berth-claude-auth",
		"docker volume rm berth-codex-gh-auth",
...
func TestOutputCommand_Default_StoppedUsesSessionMessage(t *testing.T) {
	...
	fake.SetResponse(`docker inspect --format {{index .Config.Labels "berth.repo-display"}}`, "org/repo\n")
	fake.SetResponse("docker inspect --format {{.State.Running}}", "false\n")
	fake.SetResponse("bash -c rm -rf '/tmp/berth-logs-"+containerName+"'", "")
	fake.SetResponse("docker cp "+containerName+":/home/agent/.claude", "")
	fake.SetResponse("bash -c find /tmp/berth-logs-"+containerName, "/tmp/berth-logs-agent-claude-test/.claude/projects/-workspace-org-repo/test.jsonl\n")
...
func TestSummaryCommand(t *testing.T) {
	...
	// InspectLabel calls
	fake.SetResponse(`docker inspect --format {{index .Config.Labels "berth.agent-type"}}`, "claude\n")
	fake.SetResponse(`docker inspect --format {{index .Config.Labels "berth.repo"}}`, "myrepo\n")
	fake.SetResponse(`docker inspect --format {{index .Config.Labels "berth.ssh"}}`, "true\n")
	fake.SetResponse(`docker inspect --format {{index .Config.Labels "berth.auth"}}`, "shared\n")
	fake.SetResponse(`docker inspect --format {{index .Config.Labels "berth.docker"}}`, "off\n")
	fake.SetResponse(`docker inspect --format {{index .Config.Labels "berth.network"}}`, "bridge\n")
	fake.SetResponse(`docker inspect --format {{index .Config.Labels "berth.resources"}}`, "cpu=4,mem=8g\n")
	fake.SetResponse(`docker inspect --format {{index .Config.Labels "berth.terminal"}}`, "tmux\n")
...
func TestDiffCommand_StoppedUsesCopiedWorkspace(t *testing.T) {
	...
	fake.SetResponse("docker ps -a --filter name=^agent-", containerName+"\n")
	fake.SetResponse("docker inspect --format {{.State.Running}}", "false\n")
	fake.SetResponse("bash -c rm -rf '/tmp/berth-workspace-"+containerName+"'", "")
	fake.SetResponse("docker cp "+containerName+":/workspace", "")
	fake.SetResponse("bash -c repo_dir=$(find /tmp/berth-workspace-"+containerName+"/workspace", "diff --git a/file b/file\n")
...
func TestCheckpointCreate_LabelIsArgSafeAndTagSafe(t *testing.T) {
	...
	if !strings.Contains(joinedCommit, "berth-checkpoint:agent-claude-test-ship-touch-tmp-pwned") {
...
func TestFleetStatus_Empty(t *testing.T) {
	...
	fake.SetResponse("docker ps -a --filter label=berth.fleet", "")
...
func TestFleetStatus_WithContainers(t *testing.T) {
	...
	fake.SetResponse("docker ps -a --filter label=berth.fleet", "agent-claude-fleet-a\tUp 2 minutes\nagent-claude-fleet-b\tExited\n")
...
func TestConfigKeyAllowed(t *testing.T) {
	allowed := []string{
		"BERTH_DEFAULT_CPUS",
		"BERTH_DEFAULT_MEMORY",
		"BERTH_DEFAULT_PIDS_LIMIT",
		"BERTH_DEFAULT_SSH",
		"BERTH_DEFAULT_DOCKER",
		"BERTH_DEFAULT_DOCKER_SOCKET",
		"BERTH_DEFAULT_REUSE_AUTH",
		"BERTH_DEFAULT_REUSE_GH_AUTH",
		"BERTH_DEFAULT_SEED_AUTH",
		"BERTH_DEFAULT_NETWORK",
		"BERTH_DEFAULT_IDENTITY",
		"GIT_AUTHOR_NAME",
		"GIT_AUTHOR_EMAIL",
		"GIT_COMMITTER_NAME",
		"GIT_COMMITTER_EMAIL",
	...
	forbidden := []string{"RANDOM_KEY", "", "BERTH_NOPE", "PATH"}
...
func TestConfigAllowedKeysList(t *testing.T) {
	...
	if !strings.Contains(list, "BERTH_DEFAULT_MEMORY") {
		t.Error("expected BERTH_DEFAULT_MEMORY in list")
	}
...
func TestConfigGetField(t *testing.T) {
	...
	tests := []struct {
		...
		{"BERTH_DEFAULT_CPUS", "4"},
		{"BERTH_DEFAULT_MEMORY", "8g"},
		{"BERTH_DEFAULT_PIDS_LIMIT", "512"},
		{"BERTH_DEFAULT_SSH", "true"},
		{"BERTH_DEFAULT_DOCKER", "false"},
		{"BERTH_DEFAULT_DOCKER_SOCKET", "false"},
		{"BERTH_DEFAULT_REUSE_AUTH", "true"},
		{"BERTH_DEFAULT_REUSE_GH_AUTH", "false"},
		{"BERTH_DEFAULT_SEED_AUTH", "true"},
		{"BERTH_DEFAULT_NETWORK", "mynet"},
		{"BERTH_DEFAULT_IDENTITY", "Alice <alice@example.com>"},
...
func TestConfigSet_ValidKey(t *testing.T) {
	...
	output := captureOutput(func() {
		if err := runConfigSet(configSetCmd, []string{"BERTH_DEFAULT_MEMORY", "16g"}); err != nil {
	...
	path := filepath.Join(xdgDir, ".berth", "config.toml")
...
func TestConfigSet_UpdateExisting(t *testing.T) {
	...
	// Pre-populate
	dir := filepath.Join(xdgDir, ".berth")
	os.MkdirAll(dir, 0755)
	os.WriteFile(filepath.Join(dir, "config.toml"), []byte("[defaults]\nmemory = \"8g\"\n"), 0644)

	captureOutput(func() {
		if err := runConfigSet(configSetCmd, []string{"BERTH_DEFAULT_MEMORY", "32g"}); err != nil {
			t.Fatalf("runConfigSet() error = %v", err)
		}
	})
...
func TestConfigGet_NotSet(t *testing.T) {
	...
	output := captureOutput(func() {
		if err := runConfigGet(configGetCmd, []string{"BERTH_DEFAULT_MEMORY"}); err != nil {
...
func TestConfigReset_NoFile(t *testing.T) {
	...
	output := captureOutput(func() {
		if err := runConfigReset(configResetCmd, []string{"BERTH_DEFAULT_MEMORY"}); err != nil {
...
func TestUserTemplatesDir(t *testing.T) {
	...
	if !strings.HasPrefix(got, filepath.Join(xdgDir, ".berth")) {
		t.Errorf("userTemplatesDir() = %q, expected prefix %q", got, filepath.Join(xdgDir, ".berth"))
	}
	if !strings.Contains(got, ".berth") {
		t.Errorf("userTemplatesDir() should contain '.berth', got: %q", got)
	}
...
func TestTemplateList_FromSymlinkedBinary(t *testing.T) {
	...
	binPath := filepath.Join(libexecDir, "berth")
	build := exec.Command("go", "build", "-o", binPath, ".")
	build.Dir = "."
	if out, err := build.CombinedOutput(); err != nil {
		t.Fatalf("build berth failed: %v\n%s", err, out)
	}

	linkPath := filepath.Join(binDir, "berth")
	if err := os.Symlink(binPath, linkPath); err != nil {
		t.Fatalf("symlink berth: %v", err)
	}
...
func TestTemplateList_NoTemplates(t *testing.T) {
	...
	os.MkdirAll(filepath.Join(xdgDir, ".berth", "templates"), 0755)
...
func TestFindTemplate_UserTemplate(t *testing.T) {
	...
	dir := filepath.Join(xdgDir, ".berth", "templates")
	os.MkdirAll(dir, 0755)
...
func TestRunAuditCommand_WithEntries(t *testing.T) {
	...
	auditDir := filepath.Join(tmpDir, "berth")
...
func TestRunTemplateCreate(t *testing.T) {
	...
	tplPath := filepath.Join(xdgDir, ".berth", "templates", "my-new-template.md")
...
func TestDiagnoseCommand_WarnsRiskyDefaults(t *testing.T) {
	...
	configDir := filepath.Join(os.Getenv("HOME"), ".berth")
...
func TestRunCostForContainer_CodexType(t *testing.T) {
	...
	fake.SetResponse("docker ps -a --filter name=^agent-", containerName+"\n")
	fake.SetResponse(`docker inspect --format {{index .Config.Labels "berth.agent-type"}}`, "codex\n")
...
func TestSpawnWithTemplate(t *testing.T) {
	...
	templateDir := filepath.Join(xdgDir, ".berth", "templates")
...
func TestExecuteSpawn_DefaultsToEphemeralAuth(t *testing.T) {
	...
	if !strings.Contains(output, "berth.auth=ephemeral") {
		t.Fatalf("expected ephemeral auth in dry-run output, got: %s", output)
	}
	if strings.Contains(output, "berth.auth=shared") {
...
func TestExecuteSpawn_DefaultDoesNotSeedHostAuth(t *testing.T) {
	...
	for _, forbidden := range []string{
		...
		"BERTH_CLAUDE_AUTH_B64=",
		"BERTH_CODEX_AUTH_B64=",
	} {
...
func TestExecuteSpawn_AppliesConfigDefaults(t *testing.T) {
	...
	t.Setenv("HOME", home)
	t.Setenv("CLAUDE_CODE_OAUTH_TOKEN", "host-token")
	configDir := filepath.Join(home, ".berth")
	if err := os.MkdirAll(configDir, 0o755); err != nil {
		t.Fatalf("mkdir config dir: %v", err)
	}
...
`
	...
	if !strings.Contains(output, "berth.docker=off") {
...
func TestSpawnWithInstructionsAndInstructionsFile(t *testing.T) {
	...
	if !strings.Contains(output, "BERTH_INSTRUCTIONS_B64=<redacted>") {
		t.Fatalf("expected redacted instructions env in dry-run output, got: %s", output)
	}
	if strings.Count(output, "BERTH_INSTRUCTIONS_B64=") != 1 {
...
func TestSpawnCodexAutoTrustWithHierarchy_DryRun(t *testing.T) {
	...
	if !strings.Contains(output, "BERTH_AUTO_TRUST=1") {
		t.Fatalf("dry-run missing auto-trust env:\n%s", output)
	}
	if !strings.Contains(output, "berth.hierarchy=display-nested/display-nested-child") {
...
func TestRunRetry_WithFeedback(t *testing.T) {
	...
	fake.SetResponse(`docker inspect --format {{index .Config.Labels "berth.ssh"}}`, "false\n")
	// InspectLabel for auth
	fake.SetResponse(`docker inspect --format {{index .Config.Labels "berth.auth"}}`, "shared\n")
	// InspectLabel for gh-auth
	fake.SetResponse(`docker inspect --format {{index .Config.Labels "berth.gh-auth"}}`, "\n")
	// InspectLabel for docker mode
	fake.SetResponse(`docker inspect --format {{index .Config.Labels "berth.docker"}}`, "off\n")
	// InspectLabel for max-cost
	fake.SetResponse(`docker inspect --format {{index .Config.Labels "berth.max-cost"}}`, "\n")
	// InspectLabel for aws
	fake.SetResponse(`docker inspect --format {{index .Config.Labels "berth.aws"}}`, "\n")
	// InspectLabel for notify, on-complete, on-fail
	fake.SetResponse(`docker inspect --format {{index .Config.Labels "berth.notify"}}`, "\n")
	fake.SetResponse(`docker inspect --format {{index .Config.Labels "berth.on-complete"}}`, "\n")
	fake.SetResponse(`docker inspect --format {{index .Config.Labels "berth.on-fail"}}`, "\n")
