...
var configCmd = &cobra.Command{
	...
	Long:    "Manage defaults in ~/.berth/config.toml so future spawns pick them up without repeating flags (e.g. default to --ssh or enable the worktree mount).",
	GroupID: groupConfig,
}
...
func runConfigKeys(cmd *cobra.Command, args []string) error {
	...
	// (BERTH_*, GIT_*) don't appear as duplicate rows.
...
func runConfigShow(cmd *cobra.Command, args []string) error {
	...
	if os.IsNotExist(err) {
		...
		fmt.Println("# Use: berth config set <key> <value>")
		return nil
	}
	if err != nil {
...
func repoTemplatesDir() string {
	...
	// Fall back to walking upward from cwd. This keeps `go run ./cmd/berth`
...
// userTemplatesDir returns ~/.berth/templates.
...
func runMCPLogin(cmd *cobra.Command, args []string) error {
	...
	fmt.Println()
	fmt.Println("  1. Start a container: berth spawn claude --repo <url>")
	fmt.Printf("  2. Then run:          berth mcp-login %s <container-name>\n", service)
	return nil
}
...
func runAWSRefresh(cmd *cobra.Command, args []string) error {
	...
	credsContent, err := inject.DecodeB64(envs["BERTH_AWS_CREDS_B64"])
	if err != nil {
