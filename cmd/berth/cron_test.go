...
func TestCronConfigPathUsesBerthHome(t *testing.T) {
	t.Setenv("HOME", "/tmp/berth-home")
	t.Setenv("XDG_CONFIG_HOME", "/tmp/berth-config")
	got := cronConfigPath()
	want := "/tmp/berth-home/.berth/cron.json"
	if got != want {
...
func TestCronConfigPathFallsBackToHome(t *testing.T) {
	...
	t.Setenv("XDG_CONFIG_HOME", "")
	got := cronConfigPath()
	want := filepath.Join(home, ".berth", "cron.json")
	if got != want {
