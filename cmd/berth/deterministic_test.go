...
// so berth CLI commands can discover it. Idempotent via sync.Once.
func ensureSharedContainer(t *testing.T) {
	...
	detOnce.Do(func() {
		...
		args := []string{
			...
			"--tmpfs", "/home/agent/.config:rw,noexec,nosuid,size=32m,uid=1000,gid=1000",
			"--tmpfs", "/home/agent/.ssh:rw,noexec,nosuid,size=1m,uid=1000,gid=1000",
			...
			// Labels so berth CLI can discover this container
			"--label", "berth.agent-type=shell",
			"--label", "berth.auth=shared",
			"--label", "berth.ssh=false",
			"--label", "berth.docker=off",
			"--label", "berth.network-mode=none",
			"--label", "berth.terminal=tmux",
			"--label", "berth.repo-display=octocat/Hello-World",
			"--label", "berth.resources=cpu=4,mem=8g,pids=512",
			detTestImage,
			"-lc", "sleep 600",
		}
...
func TestDet_SecurityPreambleInjectedByEntrypoint(t *testing.T) {
	...
	if !strings.Contains(out, "berth:security-preamble") {
...
// ═══════════════════════════════════════════════════════════════════════════
// Group 10: berth CLI Against Shared Container
// ═══════════════════════════════════════════════════════════════════════════

func TestDet_BerthList(t *testing.T) {
	ensureSharedContainer(t)
...
func TestDet_BerthSummary(t *testing.T) {
	ensureSharedContainer(t)
	out, err := runBerth(t, "summary", detSharedContainer)
	if err != nil {
		t.Fatalf("summary: %v\n%s", err, out)
	}
...
func TestDet_BerthDiff(t *testing.T) {
	ensureSharedContainer(t)
	repo := "/workspace/octocat/Hello-World"

	// Make a tracked change
	detExec(t, "bash", "-c", fmt.Sprintf("echo 'berth-diff-test' >> %s/README", repo))
	defer detExec(t, "git", "-C", repo, "checkout", "--", "README")

	out, err := runBerth(t, "diff", detSharedContainer)
	if err != nil {
		t.Fatalf("diff failed: %v\n%s", err, out)
	}
	if !strings.Contains(out, "berth-diff-test") {
		t.Fatalf("diff should show our change:\n%s", out)
	}
}

func TestDet_BerthDiffStat(t *testing.T) {
	ensureSharedContainer(t)
	repo := "/workspace/octocat/Hello-World"
	...
	out, err := runBerth(t, "diff", "--stat", detSharedContainer)
	if err != nil {
...
func TestDet_BerthTodoWorkflow(t *testing.T) {
	ensureSharedContainer(t)

	// Ensure .berth dir exists
	detExec(t, "mkdir", "-p", "/workspace/.berth")

	// Add todo
	out, err := runBerth(t, "todo", "add", detSharedContainer, "Det: write more tests")
	if err != nil {
		t.Fatalf("todo add: %v\n%s", err, out)
	}

	// List
	out, err = runBerth(t, "todo", "list", detSharedContainer)
	if err != nil {
	...
	// Check
	out, err = runBerth(t, "todo", "check", detSharedContainer, "1")
	if err != nil {
	...
	// Uncheck
	out, err = runBerth(t, "todo", "uncheck", detSharedContainer, "1")
	if err != nil {
...
func TestDet_BerthCheckpointWorkflow(t *testing.T) {
	ensureSharedContainer(t)
	repo := "/workspace/octocat/Hello-World"
	...
	out, err := runBerth(t, "checkpoint", "create", detSharedContainer, "det-cp")
	if err != nil {
		t.Fatalf("checkpoint create failed: %v\n%s", err, out)
	}

	// List checkpoints
	out, err = runBerth(t, "checkpoint", "list", detSharedContainer)
	if err != nil {
	...
	out, err = runBerth(t, "checkpoint", "restore", detSharedContainer, "0")
	if err != nil {
		t.Logf("checkpoint restore: %v\n%s", err, out)
	}
}

func TestDet_BerthOutputCommits(t *testing.T) {
	ensureSharedContainer(t)
	repo := "/workspace/octocat/Hello-World"
	...
	out, err := runBerth(t, "output", "--commits", detSharedContainer)
	if err != nil {
...
func TestDet_BerthOutputFiles(t *testing.T) {
	ensureSharedContainer(t)

	out, err := runBerth(t, "output", "--files", detSharedContainer)
	if err != nil {
...
func TestDet_BerthPeek(t *testing.T) {
	ensureSharedContainer(t)
	// The shared container runs 'sleep 300', not tmux, so peek will fail.
	// This test just verifies berth handles the error gracefully.
	_, err := runBerth(t, "peek", detSharedContainer)
	if err == nil {
...
func TestDet_BerthCost(t *testing.T) {
	ensureSharedContainer(t)
	out, err := runBerth(t, "cost", detSharedContainer)
	if err != nil {
