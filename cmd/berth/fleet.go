...
'berth pipeline' instead.`,
	Example: `  berth fleet manifest.yaml
  berth fleet manifest.yaml --dry-run
  berth fleet manifest.yaml --repo git@github.com:org/repo.git --var env=staging
  berth fleet status`,
...
const pipelineDetachedEnv = "BERTH_PIPELINE_DETACHED"
...
rejected instead of silently ignored.`,
	Example: `  berth pipeline pipeline.yaml
  berth pipeline pipeline.yaml --dry-run
  berth pipeline my-saved-pipeline --var branch=main
  berth pipeline pipeline.yaml --background`,
	Args:    cobra.ExactArgs(1),
	GroupID: groupFleet,
	RunE:    runPipeline,
}
...
func printFleetDryRunSpec(spec fleet.AgentSpec) {
	...
	fmt.Printf("  Would spawn: berth spawn %s", opts.AgentType)
