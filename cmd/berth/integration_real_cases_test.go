...
func TestE2E_RunQuickStartCreatesContainer(t *testing.T) {
	...
	if got := dockerInspectField(t, fullName, `{{index .Config.Labels "berth.agent-type"}}`); got != "claude" {
		t.Fatalf("agent-type = %q, want claude", got)
	}
	if got := dockerInspectField(t, fullName, `{{index .Config.Labels "berth.auth"}}`); got != "shared" {
		t.Fatalf("auth label = %q, want shared", got)
	}
	if got := dockerInspectField(t, fullName, `{{index .Config.Labels "berth.prompt"}}`); !strings.Contains(got, "List the repository files") {
		t.Fatalf("prompt label = %q, want prompt snippet", got)
	}
}

func TestDet_BerthOutputFilesShowsTrackedAndUntrackedChanges(t *testing.T) {
	ensureSharedContainer(t)
...
func TestDet_StoppedContainerDiffAndOutputFiles(t *testing.T) {
	...
	out, err = runBerth(t, "output", "--files", name)
	if err != nil {
