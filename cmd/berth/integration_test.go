...
var testVMName = "berth"
var integrationHome string

// binaryPath holds the path to the built berth binary.
// Set relative to the module root by TestMain.
var binaryPath string

func TestMain(m *testing.M) {
	if os.Getenv("BERTH_INTEGRATION") != "1" {
		fmt.Println("SKIP: set BERTH_INTEGRATION=1 to run integration tests")
		os.Exit(0)
	}
	if v := os.Getenv("BERTH_INTEGRATION_VM"); v != "" {
		testVMName = v
	}

	// Build the binary
	fmt.Println("Building berth binary...")
	build := exec.Command("go", "build", "-o", "../../bin/berth", "./")
	build.Dir = "" // current dir is cmd/berth/
	...
	if err == nil {
		...
		binaryPath = modDir + "bin/berth"
	} else {
		binaryPath = "../../bin/berth"
	}

	var homeErr error
	integrationHome, homeErr = os.MkdirTemp("", "berth-integration-home-")
	if homeErr != nil {
...
[git]
author_name = "Berth Test"
author_email = "berth-test@example.invalid"
committer_name = "Berth Test"
committer_email = "berth-test@example.invalid"
`
...
func integrationCommandEnv(overrides map[string]string) []string {
	managed := map[string]string{
		"BERTH_CONFIG_HOME": integrationHome,
		"BERTH_STATE_HOME":  integrationHome,
		"BERTH_VM_NAME":     testVMName,
...
func skipIntegrationHostEnv(key string) bool {
	...
	case "XDG_CONFIG_HOME", "CLAUDE_CONFIG_DIR", "CODEX_HOME",
		"BERTH_CONFIG_HOME", "BERTH_STATE_HOME",
		"BERTH_VM_NAME",
		"GIT_AUTHOR_NAME", "GIT_AUTHOR_EMAIL",
		"GIT_COMMITTER_NAME", "GIT_COMMITTER_EMAIL":
		return true
	}
	return strings.HasPrefix(key, "BERTH_DEFAULT_")
}
...
// runBerth executes the berth binary with given args and returns combined output.
func runBerth(t *testing.T, args ...string) (string, error) {
	t.Helper()
...
func TestE2E_VersionOutput(t *testing.T) {
	out, err := runBerth(t, "--version")
	if err != nil {
		t.Fatalf("version failed: %v\n%s", err, out)
	}
	if !strings.Contains(out, "berth v") {
		t.Fatalf("version output unexpected: %s", out)
	}
}

func TestE2E_HelpOutput(t *testing.T) {
	out, err := runBerth(t, "--help")
	if err != nil {
...
func TestE2E_SpawnDryRunDoesNotCreateContainer(t *testing.T) {
	...
	out, err := runBerth(t, "spawn", "claude",
		"--name", suffix,
...
func checkSpawnAgentTypeLabel(t *testing.T, fullName string) {
	v := dockerInspectField(t, fullName, `{{index .Config.Labels "berth.agent-type"}}`)
	if v != "claude" {
...
func checkSpawnSSHLabel(t *testing.T, fullName string) {
	v := dockerInspectField(t, fullName, `{{index .Config.Labels "berth.ssh"}}`)
	if v != "false" {
...
func checkSpawnAuthLabel(t *testing.T, fullName string) {
	v := dockerInspectField(t, fullName, `{{index .Config.Labels "berth.auth"}}`)
	if v != "shared" {
...
func checkSpawnTerminalLabel(t *testing.T, fullName string) {
	v := dockerInspectField(t, fullName, `{{index .Config.Labels "berth.terminal"}}`)
	if v != "tmux" {
...
func checkSpawnNetworkModeLabel(t *testing.T, fullName string) {
	v := dockerInspectField(t, fullName, `{{index .Config.Labels "berth.network-mode"}}`)
	if v != "managed" {
...
func TestE2E_SpawnCodex(t *testing.T) {
	...
	out, err := runBerth(t, "spawn", "codex",
		"--name", suffix,
		"--repo", "https://github.com/octocat/Hello-World.git",
		"--background")
	if err != nil {
	...
	agentType := dockerInspectField(t, fullName, `{{index .Config.Labels "berth.agent-type"}}`)
	if agentType != "codex" {
...
func TestE2E_SpawnWithPrompt(t *testing.T) {
	...
	prompt := dockerInspectField(t, fullName, `{{index .Config.Labels "berth.prompt"}}`)
...
func TestE2E_SpawnWithEphemeralAuth(t *testing.T) {
	...
	auth := dockerInspectField(t, fullName, `{{index .Config.Labels "berth.auth"}}`)
	if auth != "ephemeral" {
...
func TestE2E_SpawnWithCustomNetwork(t *testing.T) {
	...
	networkMode := dockerInspectField(t, fullName, `{{index .Config.Labels "berth.network-mode"}}`)
...
func TestE2E_StopRemovesContainer(t *testing.T) {
	...
	// Stop it
	out, err = runBerth(t, "stop", fullName)
	if err != nil {
...
func TestE2E_SummaryShowsInfo(t *testing.T) {
	...
	out, err = runBerth(t, "summary", fullName)
	if err != nil {
...
func TestE2E_PeekShowsOutput(t *testing.T) {
	...
	out, err := runBerth(t, "peek", fullName, "--lines", "10")
	if err != nil {
...
func TestE2E_DiffCommand(t *testing.T) {
	...
	out, err := runBerth(t, "diff", fullName)
	if err != nil {
...
func TestE2E_AuditShowsSpawnEntry(t *testing.T) {
	...
	out, err := runBerth(t, "audit", "--lines", "10")
	if err != nil {
...
func TestE2E_TodoWorkflow(t *testing.T) {
	...
	// Add a todo
	out, err = runBerth(t, "todo", "add", fullName, "Fix the tests")
	if err != nil {
	...
	// List todos
	out, err = runBerth(t, "todo", "list", fullName)
	if err != nil {
	...
	// Check it
	out, err = runBerth(t, "todo", "check", fullName, "1")
	if err != nil {
...
func TestE2E_SpawnDockerModeOff(t *testing.T) {
	...
	dockerMode := dockerInspectField(t, fullName, `{{index .Config.Labels "berth.docker"}}`)
	if dockerMode != "off" {
