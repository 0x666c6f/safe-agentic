...
func TestRunJudgeStage_Success(t *testing.T) {
	...
	// verdict persisted
	verdictPath := filepath.Join(os.Getenv("HOME"), ".berth", "state", "judge", "best-of-two-pick-20260101-000000.json")
...
func TestRunJudgeStage_AutoPR(t *testing.T) {
	...
	if !strings.HasPrefix(parts[1], "berth/judge-") {
		t.Fatalf("PR head branch = %q, want berth/judge- prefix", parts[1])
	}
	if parts[2] != "develop" {
...
func TestWinnerPRBranch(t *testing.T) {
	...
	if !strings.HasPrefix(got, "berth/judge-") {
		t.Fatalf("branch = %q, want berth/judge- prefix", got)
	}
...
func TestCreateWinnerPRImplUsesVolumesFromHelper(t *testing.T) {
	fake := vmexec.NewFake()
	fake.SetResponse("docker inspect --format {{.Config.Image}} agent-codex-b", "berth:latest")
	fake.SetResponse("docker inspect --format {{.HostConfig.NetworkMode}} agent-codex-b", "agent-codex-b-net")

	if _, err := createWinnerPRImpl(context.Background(), fake, "agent-codex-b", "berth/judge-p-pick-ts", "main", "My title", "My body"); err != nil {
	...
	// Trailing argv: ... -c <script> berth-judge-pr <branch> <base> <title> <body>
	if last[len(last)-1] != "My body" || last[len(last)-2] != "My title" {
		t.Fatalf("title/body must be argv, tail = %v", last[len(last)-4:])
	}
	if last[len(last)-3] != "main" || last[len(last)-4] != "berth/judge-p-pick-ts" {
