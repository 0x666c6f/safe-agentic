	...
	case "running":
		...
		return fmt.Errorf(
			"container %s is running but has no attachable tmux session; the agent may still be running headless.\n"+
				"Use `berth steer %s \"...\"` to send it input, or `berth stop %s` then `berth attach %s --resume` to restart and continue.",
			name, name, name, name)
		...
		if !authVolumePersists(authType) {
			...
			return fmt.Errorf(
				"container %s used ephemeral auth; its conversation transcript was tmpfs-backed and did not survive the stop, so --resume cannot recover it.\n"+
					"Use `berth attach %s` (without --resume) or `berth retry %s` for a fresh run, or re-run the task with --reuse-auth so future sessions persist.",
				name, name, name)
		}
		...
		if has, _ := tmux.HasSession(ctx, exec, name); !has {
			return fmt.Errorf(
				"container %s is still running but has no attachable tmux session; the agent may be running headless.\n"+
					"Use `berth steer %s \"...\"` to send input, or `berth stop %s` first, then `berth retry %s --resume`.",
				name, name, name, name)
		}
...
func applyReconstructedEnvs(opts *SpawnOpts, getEnv func(string) string) {
	...
	opts.Prompt = decodeB64Value(getEnv("BERTH_PROMPT_B64"))
	opts.Instructions = decodeB64Value(getEnv("BERTH_INSTRUCTIONS_B64"))
	opts.Template = decodeB64Value(getEnv("BERTH_TEMPLATE_B64"))
	opts.OnExit = decodeB64Value(getEnv("BERTH_ON_EXIT_B64"))
}
