...
Source: the raw on-screen terminal (running containers only). Compare with:
  ...
  replay   the structured event-log timeline`,
	Example: `  berth peek my-task            # last 30 pane lines of one agent
  berth peek --latest --lines 50  # more lines from the newest agent`,
...
(structured event log).`,
	Example: `  berth logs my-task              # last 50 rendered turns
  berth logs --latest --lines 100  # more history from the newest agent
  berth logs my-task -f            # follow the live transcript`,
	Args:    cobra.MaximumNArgs(1),
	GroupID: groupObserve,
	RunE:    runLogs,
}
...
`
	...
	tmpDir := "/tmp/berth-logs-" + strings.ReplaceAll(name, "/", "-")
...
workspace repo; for a fuller diff use the top-level 'berth diff'.`,
	Example: `  berth output my-task          # last agent message
  berth output --latest --json  # message + status as JSON
  berth output my-task --files  # files the agent changed
  berth output my-task --diff   # quick git diff (see also: berth diff)`,
	Args:    cobra.MaximumNArgs(1),
	GroupID: groupObserve,
	RunE:    runOutput,
}
...
func init() {
	outputCmd.Flags().BoolVar(&outputDiff, "diff", false, "Show a git diff of the workspace instead of the message ('berth diff' is the fuller form)")
...
func readTmuxPaneOutput(ctx context.Context, exec vmexec.Executor, name string, running bool) ([]byte, bool) {
	...
	termLabel, _ := docker.InspectLabel(ctx, exec, name, "berth.terminal")
	if termLabel != "tmux" {
		return nil, false
	}
...
func runGitOnStoppedWorkspace(ctx context.Context, exec vmexec.Executor, name, gitCmd string) ([]byte, error) {
	tmpDir := "/tmp/berth-workspace-" + strings.ReplaceAll(name, "/", "-")
	if _, err := exec.Run(ctx, "bash", "-c", "rm -rf "+shellQuote(tmpDir)+" && mkdir -p "+shellQuote(tmpDir)); err != nil {
		return nil, fmt.Errorf("create temp dir: %w", err)
	}
	defer exec.Run(context.Background(), "bash", "-c", "rm -rf "+shellQuote(tmpDir))
...
Source: container metadata only (docker inspect state + berth labels) —
...
var auditCmd = &cobra.Command{
	...
	Long: `Print recent entries from the host-side audit log (~/.berth/state/audit.jsonl):
every berth operation with its timestamp, action, container, and details.
...
Exports the unrendered files; to read them in place use 'logs' or 'output'.`,
	Example: `  berth sessions my-task              # export to ./agent-sessions/my-task
  berth sessions my-task ~/backups/    # export to a chosen directory
  berth sessions --latest ~/sessions/  # export the newest agent`,
...
Source: /workspace/.berth/session-events.jsonl (the berth event
...
func runReplay(cmd *cobra.Command, args []string) error {
	...
	out, err := exec.Run(ctx, "docker", "exec", name,
		"bash", "-c", "cat /workspace/.berth/session-events.jsonl 2>/dev/null || true")
	if err != nil {
