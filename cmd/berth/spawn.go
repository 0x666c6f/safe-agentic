...
defaults, use 'berth run' instead.`,
	Example: `  berth spawn claude --ssh --repo git@github.com:org/repo.git
  ...
  berth spawn claude --repo https://github.com/org/repo.git --network agent-isolated
  berth spawn claude --background --on-complete 'berth pr --latest' --repo git@github.com:org/repo.git`,
...
'spawn' for the common case; use 'spawn' when you need full flag control.`,
	Example: `  berth run git@github.com:org/repo.git "Fix the CI tests"
  berth run https://github.com/org/repo.git "Add unit tests"
  berth run git@github.com:org/repo.git   # no prompt: interactive agent`,
...
func init() {
	...
	f.StringVar(&spawnOpts.Template, "template", "", "Use a named prompt template instead of --prompt (see 'berth template list')")
	...
	f.BoolVar(&spawnOpts.AllowSetupScripts, "allow-setup-scripts", false, "Run repo-provided setup hooks (berth.json .scripts.setup) in the container")
	...
	f.StringVar(&spawnOpts.Notify, "notify", "", "Notify when done: comma-separated targets — terminal, system, slack:<webhook>, command:<cmd> (runs on host with $BERTH_CONTAINER/$BERTH_MESSAGE)")
	...
	rf.StringVar(&spawnOpts.Template, "template", "", "Use a named prompt template instead of the trailing prompt (see 'berth template list')")
	...
	rf.BoolVar(&spawnOpts.AllowSetupScripts, "allow-setup-scripts", false, "Run repo-provided setup hooks (berth.json .scripts.setup) in the container")
...
func appendPromptAndTemplate(cmd *docker.DockerRunCmd, opts SpawnOpts) error {
	...
	if opts.Prompt != "" {
		cmd.AddEnv("BERTH_PROMPT_B64", inject.EncodeB64(opts.Prompt))
	...
	if instructions != "" {
		cmd.AddEnv("BERTH_INSTRUCTIONS_B64", inject.EncodeB64(instructions))
		cmd.AddLabel(labels.Instructions, "1")
	}
	if opts.Template != "" {
		cmd.AddEnv("BERTH_TEMPLATE_B64", inject.EncodeB64(opts.Template))
	}
	return nil
}
...
func appendCallbacksAndMetadata(cmd *docker.DockerRunCmd, opts SpawnOpts) {
	if opts.OnExit != "" {
		...
		cmd.AddEnv("BERTH_ON_EXIT_B64", inject.EncodeB64(opts.OnExit))
	}
	if opts.OnComplete != "" {
		cmd.AddLabel(labels.OnCompleteB64, inject.EncodeB64(opts.OnComplete))
		cmd.AddEnv("BERTH_ON_COMPLETE_B64", inject.EncodeB64(opts.OnComplete))
	}
	if opts.OnFail != "" {
		cmd.AddLabel(labels.OnFailB64, inject.EncodeB64(opts.OnFail))
		cmd.AddEnv("BERTH_ON_FAIL_B64", inject.EncodeB64(opts.OnFail))
	}
	if opts.MaxCost != "" {
...
// launchDetachedNotifyWait forks `berth notify-wait <container>` so the
...
func confirmInteractiveSpawn(opts SpawnOpts, resolved spawnResolved) error {
	...
	// Prompt humans only. Programmatic callers (desktop app, berth-claude/
