...
func TestSpawnDryRunContainsSecurityFlags(t *testing.T) {
	...
	cmd := docker.NewRunCmd(containerName, "berth:latest")
...
func TestStartSpawnContainerStartsDinDBeforeAgent(t *testing.T) {
	fake := vmexec.NewFake()
	fake.SetResponse("docker exec berth-docker-agent-claude-test docker info", "ok")
	cmd := docker.NewRunCmd("agent-claude-test", "berth:latest")
	...
	if !strings.Contains(first, "berth-docker-agent-claude-test") {
...
func worktreeUnderRoot(home, leaf string) (string, string) {
	return filepath.Join(home, ".berth", "worktrees", leaf), "/worktrees/" + leaf
}
...
func TestExecuteSpawnWorktreeDryRun(t *testing.T) {
	...
	output := captureOutput(func() {
		err := executeSpawn(SpawnOpts{
			...
			WorktreeBranch: "berth/worktree-test",
			DryRun:         true,
		})
		if err != nil {
...
func TestExecuteSpawnWorktreeRejectsOutOfRootPath(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "outside the berth worktrees root") {
...
func TestExecuteSpawnPolicyDeniesDockerBeforeDryRun(t *testing.T) {
	...
	home := t.TempDir()
	t.Setenv("HOME", home)
	rulesDir := filepath.Join(home, ".berth")
...
func TestAppendAuthVolumes_ShellEphemeralMountsClaudeAndCodex(t *testing.T) {
	cmd := docker.NewRunCmd("agent-shell-test", "berth:latest")
