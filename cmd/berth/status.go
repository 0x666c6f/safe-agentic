...
func init() {
	statusCmd.Flags().BoolVar(&statusAll, "all", false, "Show all berth containers")
...
// listAgentContainerNames returns all berth container names.
...
// entry for each one currently blocked, so `berth inbox` surfaces agents that
