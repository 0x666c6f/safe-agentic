...
lighter 'berth output --diff' shares the same git command but adds no flags).`,
	Example: `  berth diff my-task            # full diff of the agent's workspace
  berth diff --latest --stat    # just the changed-file summary
  berth diff my-task -s         # side-by-side view via delta`,
...
func runCheckpointCreate(cmd *cobra.Command, args []string) error {
	...
	imageTag := fmt.Sprintf("berth-checkpoint:%s-%s", name, checkpointTagSuffix(label))
...
func readTodos(ctx context.Context, exec vmexec.Executor, containerName string) ([]todoItem, error) {
	out, err := exec.Run(ctx, "docker", "exec", containerName,
		"bash", "-c", "cat /workspace/.berth/todos.json 2>/dev/null || echo '[]'")
	if err != nil {
...
func writeTodos(ctx context.Context, exec vmexec.Executor, containerName string, items []todoItem) error {
	...
	writeCmd := "mkdir -p /workspace/.berth && printf %s \"$1\" | base64 -d > /workspace/.berth/todos.json"
