...
func TestMachineCreateArgsHomeMount(t *testing.T) {
	...
	none := strings.Join(machineCreateArgs("safe-agentic", "none"), " ")
	if !strings.Contains(none, "--home-mount none") {
		t.Fatalf("machineCreateArgs(none) = %q, want --home-mount none", none)
	}
	for _, want := range []string{"machine create alpine:3.22", "--name safe-agentic", "--cpus 4", "--memory 8G"} {
	...
	rw := strings.Join(machineCreateArgs("safe-agentic", "rw"), " ")
...
func TestValidateWorktreesUnderHome(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	if err := validateWorktreesUnderHome(filepath.Join(home, ".safe-ag", "worktrees")); err != nil {
