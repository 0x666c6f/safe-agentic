...
func setStateResponses(fake *vmexec.FakeExecutor, running bool, agentType, terminal string) {
	...
	fake.SetResponse(`docker inspect --format {{index .Config.Labels "safe-agentic.agent-type"}}`, agentType+"\n")
	fake.SetResponse(`docker inspect --format {{index .Config.Labels "safe-agentic.terminal"}}`, terminal+"\n")
}
