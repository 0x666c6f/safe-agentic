...
cp "$SRC_REPO_DIR/bin/docker-runtime.sh" "$TEST_REPO/bin/docker-runtime.sh"
...
(
  ...
  printf 'gone\n' > gone.txt
  printf 'ignore me\n' > scratch.txt
  git add -- bin/agent bin/agent-alias bin/agent-lib.sh bin/docker-runtime.sh bin/repo-url.sh config/seccomp.json vm/setup.sh tracked.txt nested/keep.txt gone.txt 'space name.txt' '--leading-dash.txt'
  git commit -qm "init"
  rm -f gone.txt
)
...
assert_file_exists "$VM_DIR/safe-agentic/bin/docker-runtime.sh" "tracked docker-runtime helper copied"
