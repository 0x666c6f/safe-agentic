...
func (h *Header) Update(running, total int, staleSince time.Time) {
	if !staleSince.IsZero() {
		...
		h.view.SetText(fmt.Sprintf(
			" [white::b]⚠ VM UNREACHABLE[white::-] — data stale since %s. Press [white::b]S[white::-] to run 'berth vm start'.    agents: %d/%d",
	...
	h.view.SetText(fmt.Sprintf(
		" [%s::b]berth[-::-]        ctx: %s VM    [::d]⏱ %ds[-::-]    agents: [%s]%d[-]/%d",
		colorToTag(colorTitle),
		vmName,
		pollInterval,
